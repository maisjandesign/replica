(() => {
  ...
  async function showLauncher() {
    ...
    shadow.innerHTML = `
      ...
      <div class="scrim"></div><section class="card" role="dialog" aria-modal="true" aria-labelledby="cc-launch-title"><div class="content">
        <header class="top"><strong class="brand">CODEX CAPTURE <span class="version">— V0.7.2</span></strong><button class="icon close" aria-label="Close"><svg viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18"/></svg></button></header>
        <div class="hero"><div class="status"><i></i>Ready to capture</div><h1 id="cc-launch-title">Turn any interface<br>into a Codex prompt.</h1><p>Capture pixels, structure, motion and visual rules in one pass.</p></div>
  ...
  function showPromptCard(data, preferences) {
    ...
    shadow.querySelector(".download").addEventListener("click", () => {
      ...
      const link = document.createElement("a"); link.href = url; link.download = `codex-capture-${Date.now()}.json`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
    });
